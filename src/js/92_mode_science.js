...
// scroll picks which of the 3 stages is showing; each stage's own slider drives its parameter
let sciAmplitude = 0.5, sciFreqT = 0.5, sciActiveStage = 0;
function renderSciActiveStage(){
  const panel = document.getElementById('sciStagePanel'); if(!panel) return;
  if(sciActiveStage === 0) renderSciAmplitudeStage(panel, { amplitude: sciAmplitude });
  else if(sciActiveStage === 1) renderSciWaveformStage(panel, { amplitude: sciAmplitude });
  else renderSciWaveDemo(panel, { freqT: sciFreqT });
  const ampGroup = document.getElementById('sciAmpGroup'), freqGroup = document.getElementById('sciFreqGroup');
  if(ampGroup) ampGroup.style.display = sciActiveStage < 2 ? '' : 'none';
  if(freqGroup) freqGroup.style.display = sciActiveStage === 2 ? '' : 'none';
}
function wireSciScrollStage(){
  ...
  if(home && wrap) home.addEventListener('scroll', () => {
    ...
    const stage = Math.min(2, Math.floor(Math.max(0, home.scrollTop - base) / STAGE_PX));
    if(stage !== sciActiveStage){ sciActiveStage = stage; renderSciActiveStage(); }
  });
  const ampSlider = document.getElementById('sciAmpSlider');
  if(ampSlider) ampSlider.addEventListener('input', () => { sciAmplitude = ampSlider.value/100; renderSciActiveStage(); });
  const freqSlider = document.getElementById('sciFreqSlider');
  if(freqSlider) freqSlider.addEventListener('input', () => {
    sciFreqT = freqSlider.value/100; renderSciActiveStage();
    const pc = Math.min(11, Math.floor(sciFreqT*12));
    if(pc !== sciWaveLastPc){ sciWaveLastPc = pc; playFreqs([m2f(60+pc)], 0.3); }
  });
  renderSciActiveStage();
}
